...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between">
            <h6 class="m-0 mt-1 font-weight-bold text-primary">Units</h6>
            <button type="button" class="btn btn-primary btn-sm" id="btnNewUnits">
                <i class="fas fa-plus-square"></i>
                Add New Units
            </button>
        </div>
        <div class="card-body">
            <table id="tableUnits" class="table table-hover table-bordered table-striped display nowrap" style="width:100%">
            </table>
        </div>
    </div>
...
<script>
    const ENTITY_API = 'units';
    const TABLE_APP = $('#tableUnits');

    const MODAL_APP = $('#modalUnits');
    const MODAL_FORM_APP = $('#formUnits');
    const MODAL_TITLE_APP = $('.modal-title');
    const MODAL_SUBMIT_APP = $('.btn-submit');

    const IMG_THUMB_APP = $('#thumbsImageUnits');
    const IMG_LABEL_APP = $('#labelFileUnits');
    const IMG_INPUT_APP = $('#customFileUnits')

    const INPUT_MASK_BIRTHDATE = $("#birthdate")

    INPUT_MASK_BIRTHDATE.inputmask("9999-99-99", {
    ...
    */
    ...
    function renderUpdate(data) {
        ...
        $('#units').val(data.result.unit)
        $('#abbreviation').val(data.result.abbreviation)
    }
...
<?= $this->section('modal') ?>
<!-- Modal -->
<div class="modal fade" id="modalUnits" tabindex="-1" role="dialog" aria-labelledby="labelModalUnits" aria-hidden="true" data-mdb-backdrop="static" data-mdb-keyboard="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="labelModalUnits"></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form autocomplete="off" id="formUnits">
                    <div class="form-group row">
                        <div class="col">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" id="units" name="unit" placeholder="Unit">
                            </div>
                        </div>
                    </div>
                    <div class="form-group row">
